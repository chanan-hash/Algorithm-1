public class StrassensMatrixMult {
    ...
    //simple matrix multiplication, row by column.
    //O(n^3)
    public static int[][] matMult(int[][] A, int[][] B) {
        int[][] C = new int[A.length][B[0].length]; // going over each row in a
        for (int i = 0; i < A[0].length; i++) { // and multiplying it by every column in B
    ...
    /**
     * Assumption - all of our matrices will be in size of nXn, so that n is a power of 2 (1X1, 2X2, 4X4, 8X8, 16X16, etc...)
     * if not, we'll complete/fill the matrices by 0, so it will be in a size of power of 2
     * <p>
     * here's a link for implementation of Strassen algorithm (from "geeks-for-geeks"):
